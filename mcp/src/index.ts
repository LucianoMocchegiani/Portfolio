...
/**
 * `/health` público, `/auth/me` stub (chat-api exige introspect URL), `/mcp` con Bearer.
 */
export function createApp(): Hono {
  const app = new Hono();

  app.get('/health', (c) =>
    c.json({ status: 'ok', checkedAt: new Date().toISOString() }),
  );
  ...
  app.all('/mcp', async (c) => {
    ...
    const authInfo = { token, clientId: 'public', scopes: [] as string[] };
...
serve({ fetch: app.fetch, port: config.port }, (info) => {
  console.log(`luciano-mcp listening on :${info.port}`);
});
